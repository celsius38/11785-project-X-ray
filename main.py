...
def get_traindata(i, downsample = 4):
    """
    @Param:
        int i: index of train set 
        downsample: downsample ratio, e.g.: 2, then height and width are divided by 2
    @Return:
        DataLoader of train data with index i
    """ 
    global args
    data, label = np.load("data/train{}.npy".format(i)), np.load("data/train_label{}.npy".format(i)) 
...
def get_valdata(downsample = 4):
    ...
    """
    global args
    data, label = np.load("data/val{}.npy".format(i)), np.load("data/val_label{}.npy".format(i)) 
    data = data[:, ::downsample, ::downsample]
    return DataLoader(  CustomDataset(data, label), 
                        batch_size = args["batch_size"], 
                        shuffle = False,
...
def get_testdata(i, downsample = 4): 
    """
    @Param: 
        int i: index of test set
        downsample: downsample ratio, e.g.: 2, then height and width are divided by 2
    @Return:
        DataLoader of test data
    """
    global args
    data, label = np.load("data/test{}.npy".format(i)), np.load("data/test_label{}.npy".format(i)) 
    data = data[:, ::downsample, ::downsample]
    return DataLoader(  CustomDataset(data, label), 
                        batch_size = args["batch_size"], 
                        shuffle = False,
